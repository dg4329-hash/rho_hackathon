...
1. **Join with the one-liner** (10 min): from your clone, `curl -fsSL https://<host>/install.sh | bash -s -- <room>` (handle from git). Accept: `node ~/.mesh/mesh.mjs status` → `● running`; `grep emit.js .claude/settings.json` in that clone; after a Claude Code restart, `/mcp` shows `mesh`.
...
6. **Pitch** (1 h): 90-second script from NEXT.md, the honest-limitations slide (room-name auth, `sh -c`, OAuth MCPs, 90 s dialog timeout, Codex/Cursor pull-only messages, one restart), the Q&A cards incl. "why a system dialog". Accept: delivered twice at ≤ 1:30 without the feed, then twice with it during rehearsal (NEXT.md item 8); "actual" timings written into DEMO.md §1.
