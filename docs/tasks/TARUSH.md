...
## Done since (2026-09-12, night)
Windows one-liner (PowerShell), the MessageBox approval dialog and the message-box notification are verified on your real box. Installers now stop + restart a running daemon; the room page also offers a downloadable `mesh-join-<room>.cmd`. Shipping tonight: the **overlay** (room page → **Pop out overlay**, Chrome/Edge) — your approval surface for the demo, OS dialog as fallback — and `wait_for_events` for Codex (*"Codex, watch mesh for the next 10 minutes"*).

## Next (post-merge)
Source of truth for ordering and acceptance: `docs/NEXT.md`. Your items, in order. Item 2 (Figma token + file key) is the biggest open item left and only you can do it.

0. **Overlay on your box** (NEXT.md item 1, 30 min, once Dev says it's up): room page → **Pop out overlay**; Dev asks `echo hi`. Accept: card appears, **Approve** → Dev gets `hi`; **Deny** → `owner declined`; close the overlay, ask again → the MessageBox appears; a message from Dev shows in the overlay and the reply box sends back.
1. ~~Windows one-liner on your real box~~ **done**. Kept for the fallback commands: PowerShell: `& ([scriptblock]::Create((irm -Headers @{'ngrok-skip-browser-warning'='1'} https://<host>/install.ps1))) <room> --as tarush`. Then `node $env:USERPROFILE\.mesh\mesh.mjs status`. Dev asks `echo hi`. Accept: installer ends with `mesh running in the background`; `status` → `● running`; the MessageBox pops; **Yes** → Dev gets `hi`; **No** → `owner declined`; a message from Dev shows a balloon. If PowerShell fails, paste the error into Evidence and use Git Bash: `curl -fsSL https://<host>/install.sh | bash -s -- <room> --as tarush` (proven). Last resort: `MESH_APPROVE=tty` in a real console, or the owner role on a Mac.
...
5. **Stage machine hygiene + rehearsal** (30 min + NEXT.md item 8). No sleep on AC, charger in, notifications ON (mesh uses them), nothing covering the dialog, `mesh log` open in a side window during rehearsal 1 only. Accept: two clean rehearsal runs; overlay answered within two minutes every time (dialog fallback gets 90 s more).
