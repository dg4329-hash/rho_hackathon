...
**Approval rule.** Prompts are serialized, one at a time. Order: (0) **watcher** — if anything long-polled `GET /pending` in the last 60 s (the room-page overlay, or `mesh watch` run by the Claude Code plugin monitor), the request is parked in the pending queue and answered via `POST /decide` (overlay buttons) or the `approve_request` tool (Claude Code's own permission prompt); no answer in 120 s, or the watcher stops polling → fall through; (1) native OS dialog — macOS `osascript display dialog`, Windows PowerShell `MessageBox`, Linux `zenity` — buttons Deny/Approve, 90 s timeout → `denied` with reason `owner did not answer in 90 s`; (2) if no dialog is possible (or `MESH_APPROVE=tty`), a terminal `[y/n]` single keypress; (3) no TTY either → `denied`, reason `no tty and no dialog available`. `MESH_APPROVE=watcher|dialog|tty` forces one surface.
**Approvals mode** (where requests and notifications go; the owner picks it in the overlay, `POST /approvals` §5): `auto` ("Both", the default) — as above: any watcher (overlay or Claude Code monitor) parks requests, first answer wins, dialog fallback. `overlay` ("Overlay only") — only `consumer=overlay` polls see and park requests; the Claude Code watcher (`mesh watch`, `wait_for_events`) gets neither requests nor message/file lines (messages stay unread for `inbox` and still show in the overlay); no overlay polled in 60 s → straight to the dialog. `agent` ("Claude Code only") — only the default (agent) consumer sees and parks requests; the overlay's polls do not count as a watcher and get `pending: []` (still get messages); no agent watcher in 60 s → straight to the dialog (no 120 s wait). `/decide` / `approve_request` answer a parked request in every mode. `dialog` — dialog (tty fallback) only; nobody sees parked requests. `tty` is still accepted for back-compat (terminal prompt) but not listed. Precedence: the owner's saved choice > `MESH_APPROVE` (when it names a mode) > `auto`. The choice is saved to `~/.mesh/approvals.json` (`{ mode, updatedAt }`) and restored at daemon start; it survives `leave` / `stop` / re-join. Deny from a dialog is `owner declined` (no typed reason); the overlay and `approve_request` may carry a short `reason`. Incoming `message` events trigger a native notification (macOS `display notification`, Linux `notify-send`, Windows a small always-on-top message box; `MESH_TOAST=1` for a toast), skipped for replayed history older than 5 min, and wake every `/pending` long-poller (overlay, watcher, `wait_for_events`).
...
- `GET /pending?wait=25&messages=1&consumer=overlay&since=<ISO>` — `{ pending: PendingRequest[], messages: InboxMessage[] }`; long-poll ≤ 25 s, returns early on a new request or message. Any caller counts as an attached watcher for 60 s (§2 approval rule), tracked per consumer: `consumer=overlay` is the overlay, anything else the agent watcher (this matters in `overlay` / `agent` approvals modes). `consumer=overlay` (+ optional `since`) returns messages newer than `since` without marking them read; the default consumer (the Claude Code watcher) marks them read, and gets `messages: []` (nothing marked) in `overlay` mode. Without `messages=1` → `{ pending }` for that consumer.
- `GET /approvals` → `{ mode, modes: ['auto','overlay','agent','dialog'] }` (`mode` is the effective mode, §2; may be `tty` via `MESH_APPROVE` or a back-compat POST). `POST /approvals` `{ mode }` (one of `auto|overlay|agent|dialog|tty`) → `{ ok: true, mode }`, saved to `~/.mesh/approvals.json`; 400 `{ ok: false, error }` for anything else. Wakes every `/pending` long-poll so it re-evaluates.
...
- `GET /health` — `{ approvals, user, room, relay: 'connected'|'disconnected', members: n, owner: boolean }` (`approvals`: the effective approvals mode, same as `GET /approvals`.mode) (`owner`: this daemon holds the room's owner token, §1a). Also how `mesh status` / `--background` decide the daemon is alive.
...
- Env: `MESH_APPROVE=tty` forces the terminal prompt (any approvals mode name works; a mode saved from the overlay wins, §2); `MESH_DEBUG=1` logs every frame; `MESH_RELAY` default relay.
