...
Windows one-liner, MessageBox dialog and message-box notification verified on Tarush's real box. Plugin on main and auto-installed by `mesh join` from `<relay>/plugin.tgz`; `mesh watch` forwards messages live. Chained commands never inherit `always`. Installers stop + restart; `.cmd`/`.command` downloads. This Mac imports Playwright (24) + filesystem (14). Shipping tonight: overlay (`docs/OVERLAY-API.md`) and `wait_for_events` (CONTRACT §3, ten tools).

## Next (post-merge)
Source of truth for ordering and acceptance: `docs/NEXT.md`. Your items, in order.

0. **Overlay + `wait_for_events` end to end** (NEXT.md items 1-2, 1.5 h): overlay Approve → output back; Deny → `owner declined`; close overlay → OS dialog fallback on Tarush's Windows box; Codex *"watch mesh for the next 10 minutes"* → reports the message and the pending request, never approves. If shipped shapes differ from `docs/OVERLAY-API.md`, fix CONTRACT §3/§5 and tell the others.
1. ~~Windows one-liner with Tarush~~ **done** (verified on real Windows). Original text kept for the fallback commands: You run `pnpm -F daemon start ask tarush "echo hi" --why t --room <room> --as dev --relay wss://<host>` while he watches for the MessageBox. Also send him a `send_message` from Claude Code. Accept: `hi` / `exit 0`, then a Deny → `denied: owner declined` exit 2; balloon notification on his side. If PowerShell fails, get him on Git Bash (proven) and file the PowerShell error in TARUSH.md.
2. **Demo repo + Claude Code wiring** (NEXT.md item 3, 1 h). Small app with `src/onboarding/Step1.tsx`. Run `pnpm -F daemon start join https://<host>/r/<room> --as dev` **from inside that repo** so the plugin (project scope) lands there; `CLAUDE.md` line telling the agent it has no Figma/Supabase access and must use the `mesh` MCP tools. If your own `mobbin`/`posthog` servers must not be offered, a `team.json` with `"import": { "servers": [] }` and an absolute `--config`. Accept: fresh `claude` → `/mcp` shows the plugin's mesh tools; the demo prompt *"Implement onboarding step 2 to match the Figma frame Onboarding/Step 2."* triggers `list_teammates → describe_capability → ask_teammate` carrying `figma-export.sh "Onboarding/Step 2"` unprompted in 3/3 fresh sessions.
...
7. **Plugin interactive 3/3** (`docs/PLUGIN.md` "Not verified"): a teammate's request interjects in a fresh interactive session, **1** approves, No → `denied`; once in auto mode. Not a blocker — the overlay is the demo surface.
